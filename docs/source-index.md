# Source Index

This index lists the primary sources used to seed the repository. Prefer these links over mirrors and AI summary pages.

## Core Papers and Benchmarks

| ID | Source | Primary URL | Code / Data |
| --- | --- | --- | --- |
| P001 | GEO: Generative Engine Optimization | https://arxiv.org/abs/2311.09735 | https://github.com/GEO-optim/GEO |
| P002 | C-SEO Bench: Does Conversational SEO Work? | https://arxiv.org/abs/2506.11097 | https://github.com/parameterlab/c-seo-bench |
| P002-v | C-SEO Bench OpenReview page | https://openreview.net/forum?id=oTeixD3oZO | https://huggingface.co/datasets/parameterlab/c-seo-bench |
| P003 | AutoGEO / What Generative Search Engines Like | https://openreview.net/forum?id=K8EinVWtUB | https://github.com/cxcscmu/AutoGEO |
| P003-a | AutoGEO arXiv | https://arxiv.org/abs/2510.11438 | https://huggingface.co/datasets/cx-cmu/GEO-Bench |
| P019 | From Experience to Skill: Multi-Agent GEO via Reusable Strategy Learning | https://arxiv.org/abs/2604.19516 | https://github.com/Wu-beining/MAGEO; https://huggingface.co/datasets/WuBeiNing/MSME-GEO-Bench |
| P004 | CC-GSEO-Bench | https://arxiv.org/abs/2509.05607 | https://github.com/qychen2001/CC-GSEO-Bench |
| P007 | SAGEO Arena | https://arxiv.org/abs/2602.12187 | not verified |
| P008 | Diagnosing and Repairing Citation Failures in GEO | https://arxiv.org/abs/2603.09296 | not verified |
| P009 | AgenticGEO | https://arxiv.org/abs/2603.20213 | https://github.com/AIcling/agentic_geo |
| P020 | E-GEO: A Testbed for GEO in E-Commerce | https://arxiv.org/abs/2511.20867 | https://github.com/psbagga17/E-GEO |

## Adjacent Papers

| ID | Source | Primary URL | Why Included |
| --- | --- | --- | --- |
| P005 | Role-Augmented Intent-Driven GSEO | https://arxiv.org/abs/2508.11158 | intent and role-based optimization |
| P006 | Generative Engine Optimization: How to Dominate AI Search | https://arxiv.org/abs/2509.08919 | empirical AI search sourcing strategy |
| P010 | Beyond Retrieval | https://arxiv.org/abs/2604.03656 | deterministic agent routing direction |
| P011 | GEM-Bench | https://arxiv.org/abs/2509.14221 | generative engine marketing and ads |
| P016 | What Evidence Do Language Models Find Convincing? | https://arxiv.org/abs/2402.11782 | evidence preference in RAG |
| P017 | ConflictBank | https://arxiv.org/abs/2408.12076 | knowledge conflict benchmark |
| P018 | When Search Engine Services meet LLMs | https://arxiv.org/abs/2407.00128 | Search4LLM / LLM4Search background |
| P021 | BRIGHT | https://arxiv.org/abs/2407.12883 | reasoning-intensive retrieval benchmark |
| P022 | Large Language Models for Information Retrieval | https://arxiv.org/abs/2308.07107 | LLM-for-IR survey |
| P023 | Search-o1 | https://arxiv.org/abs/2501.05366 | agentic search-enhanced reasoning |
| P024 | Is ChatGPT Good at Search? | https://arxiv.org/abs/2304.09542 | LLM reranking background |

## Security and Adversarial Work

| ID | Source | Primary URL | Why Included |
| --- | --- | --- | --- |
| P012 | Adversarial SEO for LLMs | https://arxiv.org/abs/2406.18382 | preference manipulation attacks |
| P013 | Ranking Manipulation for Conversational Search Engines | https://arxiv.org/abs/2406.03589 | prompt-injection ranking manipulation |
| P014 | GASLITEing the Retrieval | https://arxiv.org/abs/2412.20953 | dense retrieval SEO attack surface |
| P015 | Dynamics of Adversarial Attacks on LLM-Based Search Engines | https://arxiv.org/abs/2501.00745 | game-theoretic attack dynamics |

## Platform and Industry Sources

| ID | Source | URL | Evidence Level |
| --- | --- | --- | --- |
| I001 | Google Search Central: succeeding in AI Search | https://developers.google.com/search/blog/2025/05/succeeding-in-ai-search | high |
| I002 | OpenAI Help Center: ChatGPT Search | https://help.openai.com/en/articles/9237897-chatgpt- | high |
| I003 | ChatGPT merchants / product discovery | https://chatgpt.com/merchants/ | high |
| I006 | GitHub topic: generative-engine-optimization | https://github.com/topics/generative-engine-optimization | medium |
| I007 | Awesome Generative Engine Optimization | https://github.com/amplifying-ai/awesome-generative-engine-optimization | medium |
| I009 | Awesome GEO | https://github.com/luka2chat/awesome-geo | medium |
